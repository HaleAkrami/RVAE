...
class RobustL21Autoencoder(object):
    ...
    def fit(self, X,X_valid,X_lab_valid, sess, learning_rate=0.15, inner_iteration = 50,
        ...
        valid_L=self.AE.getRecon(X = X_valid, sess = sess)
        valid_S=SHR.l21shrink(self.lambda_, (X_valid - valid_L).T).T
        filename1 = '/ImagePTE1/akrami/git_repos_old/lesion-detector/src/AutoEncoder/L12/original_code/RobustAutoencoder-master/experiments/Outlier Detection/l21out'
        

        np.savez(filename1 + str(self.lambda_) + '_'+ '.npz',
                 L=valid_L,
                 S=valid_S,
                 data=X_valid,
                 anom_lab=X_lab_valid)
        #valid_S.dump("l21S.npk")
