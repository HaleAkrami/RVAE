...
#batch_size = 133
mat = spio.loadmat(
    '/ImagePTE1/akrami/git_repos_old/rvae/validation/matlab/emnist-letters.mat')
data = mat['dataset']
...
def test(frac_anom, beta_val):
    ...
    with torch.no_grad():
        for i, (data, data_lab) in enumerate(test_loader):
            ...
            test_loss_anom += beta_loss_function(recon_batch* anom_lab, data* anom_lab, mu* anom_lab, logvar* anom_lab, beta=0).item()
            test_loss_total += beta_loss_function(recon_batch, data, mu, logvar, 0).item()
