    ...
    with torch.no_grad():
        for i, (data, data_lab) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                save_image(
                    comparison.cpu(),
                    'results/fashion_mnist_recon_shallow_denoising_ae_' +
                    str(std_dev) + '_' + str(frac_anom) + '.png',
                    nrow=n)

        np.savez('results/fashion_mnist_' + str(std_dev) + '_denoising_ae_' +
                 str(frac_anom) + '.npz',
...
if __name__ == "__main__":
    ...
    for b, std_dev in enumerate(srange):
        ...
        for a, frac_anom in enumerate(anrange):
            ...
            # save the model
            torch.save(
                model, 'fashion_mnist_beta_shallow_denoising_ae_' +
        ...
        np.savez('test_loss_fashionmnist_beta_shallow_denoising_ae_' + str(b) +
                 '.npz',
