...
def prob_loss_function(Y, x):
    ...
    std = SIGMA
    #K1 = -D/2 *math.log((2 * math.pi * (std**2)))



    term1 = torch.sum((((Y - x)/ std)**2), 1)
    #const2 = -(NDim / 2) * math.log((2 * math.pi))

    prob_term =  (-(0.5) * term1) #+const2
        ...
        for i, (data, data_lab) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                save_image(comparison.cpu(),
                           'KBS_results/cp_fashion_mnist_recon_shallow_' +
                           str(beta_val) + '_' + str(frac_anom) + '.png',
                           nrow=n)

        np.savez('KBS_results/cp_fashion_mnist_' + str(beta_val) + '_' +
                 str(frac_anom) + '.npz',
    ...
    with torch.no_grad():
        for i, (data, data_lab) in enumerate(val_loader):
            ...
            if i == 0:
                ...
                save_image(comparison.cpu(),
                           'KBS_results/cp_val_fashion_mnist_recon_shallow_' +
                           str(beta_val) + '_' + str(frac_anom) + '.png',
                           nrow=n)

        np.savez('KBS_results/cp_val_fashion_mnist_' + str(beta_val) + '_' +
                 str(frac_anom) + '.npz',
                 recon=recon_batch.cpu(),
                 data=data.cpu(),
                 anom_lab=anom_lab.cpu())
...
if __name__ == "__main__":
    ...
    brange=np.arange(0.0001,0.1,0.01)
    ...
    #anrange = np.arange(0, 0.2001, 0.01)
    ...
    val_loss_total = np.zeros((len(anrange), len(brange)))
    val_loss_anom = np.zeros((len(anrange), len(brange)))
    val_loss_normals = np.zeros((len(anrange), len(brange)))
    ...
    for b, betaval in enumerate(brange):
        ...
        for a, frac_anom in enumerate(anrange):
            ...
            # save the model
            torch.save(model, 'KBS_results/cp_fashion_mnist_beta_shallow_' + str(betaval) + '_frac_anom_' + str(frac_anom))
            ...
            val_loss_total[a, b], val_loss_anom[a, b], val_loss_normals[
                a, b] = val(frac_anom, beta_val=betaval)


        np.savez('KBS_results/cp_val_loss_fashionmnist_beta_shallow' + str(b) + '.npz',
        ...
        np.savez('KBS_results/cp_test_loss_fashionmnist_beta_shallow' + str(b) + '.npz',
