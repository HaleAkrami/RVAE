...
def beta_loss_function(recon_x, x, mu, logvar, beta):
    ...
    if beta > 0:
        ...
        BBCE = prob_loss_function(recon_x.view(-1, 784), x.view(-1, 784))
        ...
        for i, (data, data_lab) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                save_image(comparison.cpu(),
                           'KBS_results/fashion_mnist_recon_shallow_' +
                           str(beta_val) + '_' + str(frac_anom) + '.png',
                           nrow=n)

        np.savez('KBS_results/fashion_mnist_' + str(beta_val) + '_' +
                 str(frac_anom) + '.npz',
    ...
    with torch.no_grad():
        for i, (data, data_lab) in enumerate(val_loader):
            ...
            if i == 0:
                ...
                save_image(comparison.cpu(),
                           'KBS_results/val_fashion_mnist_recon_shallow_' +
                           str(beta_val) + '_' + str(frac_anom) + '.png',
                           nrow=n)

        np.savez('KBS_results/val_fashion_mnist_' + str(beta_val) + '_' +
                 str(frac_anom) + '.npz',
                 recon=recon_batch.cpu(),
                 data=data.cpu(),
                 anom_lab=anom_lab.cpu())
...
if __name__ == "__main__":
    ...
    for b, betaval in enumerate(brange):
        ...
        for a, frac_anom in enumerate(anrange):
            ...
            # save the model
            torch.save(model, 'KBS_results/' + str(betaval) + '_frac_anom_' + str(frac_anom))
        ...
        np.savez('KBS_results/val_loss_fashionmnist_beta_shallow' + str(b) + '.npz',
        ...
        np.savez('KBS_results/test_loss_fashionmnist_beta_shallow' + str(b) + '.npz',
