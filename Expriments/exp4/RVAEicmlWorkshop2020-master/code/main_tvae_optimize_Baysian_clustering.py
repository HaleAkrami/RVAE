...
from utils_loss import loss_function, score_dataset,score_dataset_cell
import os
...
def train(beta, regularizer, dataset, anom_frac):
    ...
    for epoch in range(epochs):
        ...
        scores_val_cell= score_dataset_cell(encoder, decoder, device, val_loader, transformer.output_info, regularizer)
        
        AUC_val = roc_auc_score(labels_val, scores_val)
        AUC_val = max(AUC_val, 1-AUC_val)
        #brent_score=hopkins_m(scores_val_cell)
...
def hopkins_m(in_X):
    ...
    CL_num,H=assess_tendency_by_metric(in_X, n_cluster=3)
