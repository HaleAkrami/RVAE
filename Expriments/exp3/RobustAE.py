...
def show_and_save(file_name,img):
    ...
    f = "/big_disk/akrami/git_repos_new/rvae_orig/validation/Brain_Imaging/figs_test/%s.png" % file_name
...
def shrink(epsilon, x):
    ...
    Returns:
        The shrunk vector
    """
    output = V(torch.zeros_like(x).cuda())
    indx1=x>epsilon
    indx2=x<-epsilon
    indx3= ((x<epsilon) & (x> -epsilon))
    output[indx1]=x[indx1]-epsilon
    output[indx2]=x[indx2]-epsilon
    output[indx3]=0
...
def BMSE_loss(Y, X, beta,sigma,Dim):
    ...
    loss1=torch.mean(term1*(K1*term3-1))
    return loss1
...
def beta_loss_function(recon_x, x, mu, beta):
    ...
    if beta > 0:
        ...
        BBCE = torch.mean(MSE_loss(recon_x.view(-1, 128*128*3),x.view(-1, 128*128*3)))
...
for epoch in range(max_epochs):
    ...
    for data in train_loader:
        ...
        S = V(torch.zeros(datav.shape,requires_grad=False).cuda())
        for iner_iter in range(iner_iter_max):
            mean,  rec_enc = G(datav-S)
            beta_err=beta_loss_function(rec_enc, datav, mean, beta) 
            err_enc = beta_err
            opt_enc.zero_grad()
            err_enc.backward()
            opt_enc.step()
            S=V(shrink(lamb_val, (datav-rec_enc)))
