...
def save_model(epoch, encoder, decoder, D):
    torch.save(decoder.cpu().state_dict(), './VAE_GAN_decoder_%d_%g.pth' % (epoch, std_dev))
    torch.save(encoder.cpu().state_dict(),'./VAE_GAN_encoder_%d_%g.pth' % (epoch, std_dev))
    torch.save(D.cpu().state_dict(), 'VAE_GAN_D_%d_%g.pth' % (epoch, std_dev))
    decoder.cuda()
    encoder.cuda()
    D.cuda()
...
def load_model(epoch, encoder, decoder, D):
    #  restore models
    decoder.load_state_dict(torch.load('./VAE_GAN_decoder_%d_%g.pth' % (epoch, std_dev)))
    decoder.cuda()
    encoder.load_state_dict(torch.load('./VAE_GAN_encoder_%d_%g.pth' % (epoch, std_dev)))
    encoder.cuda()
    D.load_state_dict(torch.load('VAE_GAN_D_%d_%g.pth' %(epoch, std_dev)))
    D.cuda()
...
for epoch in range(max_epochs):
    ...
    for data in train_loader:
        ...
        mean, logvar, rec_enc = G(datav + std_dev * torch.randn(datav.size()).cuda())  # add noise to the input
        beta_err=beta_loss_function(rec_enc, datav, mean, logvar,beta=0) 
        err_enc = beta_err
        opt_enc.zero_grad()
        err_enc.backward()
        opt_enc.step()
    ...
    show_and_save('Input_epoch_%d_%g.png' % (epoch, std_dev) ,make_grid((fixed_batch.data[:,2:3,:,:]).cpu(),8))
    show_and_save('rec_epoch_%d_%g.png' % (epoch, std_dev) ,make_grid((rec_imgs.data[:,2:3,:,:]).cpu(),8))
    samples = G.decoder(fixed_noise)
    show_and_save('samples_epoch_%d_%g.png' % (epoch, std_dev) ,make_grid((samples.data[:,2:3,:,:]).cpu(),8))
    show_and_save('Error_epoch_%d_%g.png' % (epoch, std_dev) ,make_grid((fixed_batch.data[:,2:3,:,:]-rec_imgs.data[:,2:3,:,:]).cpu(),8))
