...
def save_model(epoch, encoder, decoder, D):
    ...
    torch.save(D.cpu().state_dict(), './KBS_results/VAE_GAN_D_%d.pth' % epoch)
    decoder.cuda()
    encoder.cuda()
    D.cuda()
...
lr = 3e-4
 ...
 #beta =0.000025
beta=0.01#[0.1,0.01,0.001,0.05,1]
...
for epoch in range(max_epochs):
    ...
    for data in train_loader:
        ...
        beta_err=beta_loss_function(rec_enc.view(rec_enc.shape[0],rec_enc.shape[1]*rec_enc.shape[2]*rec_enc.shape[3]), 
        datav.view(datav.shape[0],datav.shape[1]*datav.shape[2]*datav.shape[3]), mean, logvar,beta) 
        err_enc = beta_err
        opt_enc.zero_grad()
        err_enc.backward()
        opt_enc.step()
