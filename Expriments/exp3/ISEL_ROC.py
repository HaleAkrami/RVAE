...
def load_model(epoch, encoder, decoder, D,loc):
    #  restore models
    decoder.load_state_dict(torch.load(loc+'/VAE_GAN_decoder_%d_0.0001_cvae.pth' % epoch))
    decoder.cuda()
    encoder.load_state_dict(torch.load(loc+'/VAE_GAN_encoder_%d_0.0001_cvae.pth' % epoch))
    encoder.cuda()
...
X_valid,X_train, = train_test_split(X_data, test_size=0.875, random_state=10002,shuffle=False)
...
#LM='/big_disk/akrami/git_repos_new/rvae_orig/validation/Brain_Imaging/figs_test/ISEL_RVAE'
#LM= '/big_disk/ajoshi/dvae_results/'
LM='/big_disk/ajoshi/cvar_results/KBS_results/'
##########call network##########
